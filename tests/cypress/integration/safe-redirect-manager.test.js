describe( 'Test redirect rules', () => {
	...
	it( 'Can create a rule', () => {
		...
		cy.get('#the-list td.srm_redirect_rule_to').first().should('have.text', '/test2');
		cy.get('#the-list td.srm_redirect_rule_status_code').first().should('have.text', '302');
	});

	it( 'Can redirect a simple rule request', () => {
		cy.createRedirectRule('/first-blog', '/hello-world', 'Simple rule note');
	...
	it( 'Can redirect a Regex rule request', () => {
		cy.createRedirectRule('/blog/(.*)', '/hello-world', 'Regex rule note', true);
